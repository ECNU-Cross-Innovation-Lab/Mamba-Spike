...
from data.dataset_loader import prepare_ntidigits_dataset
from models.mamba_spike import create_mamba_spike_ntidigits


class NTIDIGITSTrainer:
    def __init__(self, batch_size=32, lr=0.001, max_epochs=200):
        ...
        self.output_dir = os.path.join(
            "results",
            f"ntidigits_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        print("Loading N-TIDIGITS dataset...")
        print("⚠ Note: First load may take time (downloading dataset)")
        self.train_loader, self.test_loader, self.num_classes = prepare_ntidigits_dataset(
            batch_size=batch_size,
            num_workers=0,  # Avoid DataLoader issues
            time_window=300000,  # 300ms for audio
            dt=1000  # 1ms bins
        )
        ...
        self.model = create_mamba_spike_ntidigits().to(self.device)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Train on N-TIDIGITS dataset')
